  ...
  return (
    <div className="h-full flex flex-col bg-base-100">
            ...
            <>
              ...
              <div className='flex items-center justify-between'>
                <div className='flex items-end'>
                  <h1 className="text-2xl font-medium text-base-content mr-3">{problem.title}</h1>
                  <button 
                    className='pb-[2.4px] cursor-pointer hover:text-primary'
                    onClick={() => setIsBookmarkModalOpen(true)}
                  >
                    {isProblemBookmarked ? (
                      <svg className="w-5 h-5 text-success" fill="currentColor" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {problem.visibleTestCases?.map((testCase, index) => (
                  <div className="mt-4" key={`case${index+1}`}>
                    ...
                    <div className="overflow-x-auto p-3 rounded-md text-sm bg-base-200 text-base-content/80">
                      <code>Input: {testCase.input}</code><br />
                      <code>Output: {testCase.output}</code><br />
                  ...
                  > 
                    <ul className="space-y-1 p-2">
                      {[
                        ...
                        <li key={q.id}>
                          <a href={`/problems/${q.id}`} className="hover:underline text-primary hover:text-primary-focus">
                            {q.title}
                          </a>
                        </li>
                      ))}
              ...
              {activeTab === 'Ask AI' && <ChatInterface problem={problem} userSolution="" qaBlocks={qaBlocks} setQaBlocks={setQaBlocks} />}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="h-10 flex items-center space-x-3 px-3 border-t border-base-300 bg-base-200 text-base-content/70 flex-shrink-0">
        <button className="flex items-center space-x-1 p-1.5 rounded hover:bg-base-300" aria-label="Like problem">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
          </svg>
          <span className="text-xs">0</span>
        </button>
        <button className="p-1.5 rounded hover:bg-base-300" aria-label="Add to favorites">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="text-xs flex items-center">
          <span className="w-2 h-2 bg-success rounded-full mr-1.5"></span>3 Online
        </div>
      </div>
