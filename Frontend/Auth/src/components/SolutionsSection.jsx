    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 border-b border-base-300 bg-base-100 flex-shrink-0">
        <div className="flex items-center space-x-3">
          <h3 className="text-lg font-semibold text-base-content">Reference Solutions</h3>
        ...
        <div className="flex items-center space-x-2">
          {problem.referenceSolution.map((solution) => (
            ...
            >
              <span className="mr-1">{getLanguageIcon(solution.language)}</span>
              {solution.language}
            </button>
          ))}
        </div>
      ...
      <div className="p-4 bg-base-200 border-b border-base-300 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <span className="text-2xl">{getLanguageIcon(selectedSolutionLanguage)}</span>
            <div>
              <h4 className="font-semibold text-base-content">{selectedSolutionLanguage} Solution</h4>
              <p className="text-sm text-base-content/60">Optimal approach for this problem</p>
            </div>
          </div>
          <button 
        ...
        {currentSolution ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <svg className="w-12 h-12 mx-auto mb-4 text-base-content/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <p className="text-base-content/70">Solution not available for {selectedSolutionLanguage}</p>
            </div>
          </div>
        )}
      </div>
